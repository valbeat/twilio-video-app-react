...
describe('the DeviceSelectionScreen component', () => {
  ...
  describe('when connecting to a room', () => {
    ...
    const wrapper = shallow(<DeviceSelectionScreen name="test name" token="test room name" setStep={() => {}} />);
  ...
  it('should go back to the RoomNameScreen when the Cancel button is clicked', () => {
    ...
    const wrapper = shallow(<DeviceSelectionScreen name="test name" token="test room name" setStep={mockSetStep} />);
    wrapper.find({ children: 'Cancel' }).simulate('click');
    expect(mockSetStep).toHaveBeenCalledWith(Steps.nameStep);
  });
